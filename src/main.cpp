...
using VSPoints = segre::Hyperplanes<math::pow(PPL, N)>;
...
int main() {
	...
	VSPoints<3> vPoints3 = geometry2.computeHyperplanesFromAllVeldkampLines(vPoints2, vLines2);
	auto new_end = std::remove_if(vPoints3.nonprojective.begin(), vPoints3.nonprojective.end(), [](const auto& hyperplane) {
		return hyperplane.count() != 16;
	});
	vPoints3.nonprojective.erase(new_end, vPoints3.nonprojective.end());

	vPoints3.projectives.insert(vPoints3.projectives.end(), vPoints3.nonprojective.begin(), vPoints3.nonprojective.end());

	VLines<3> vLines3 = geometry3.computeVeldkampLines(vPoints3.projectives);
	geometry3.distinguishVeldkampLines(vLines3, vPoints3.projectives, geometry4);
	...
	std::sort(geometry3_hyp_table.begin(), geometry3_hyp_table.end(), [] (const segre::HyperplaneTableEntry& a, const segre::HyperplaneTableEntry& b) {
		return std::make_tuple(a.projective, a.nbrPoints) > std::make_tuple(b.projective, b.nbrPoints);
	});
	...
	{
		auto new_end = std::remove_if(geometry3_lin_table.begin(), geometry3_lin_table.end(), [](const segre::VeldkampLineTableEntry& a) {
			return a.pointsType.find(5) == a.pointsType.end();
		});
		geometry3_lin_table.erase(new_end, geometry3_lin_table.end());
	}

	/*std::vector<segre::VeldkampLineTableEntryWithLines<PPL>> geometry3_lin_table_with_lines = geometry3.makeVeldkampLinesTableWithLines(vLines3, vPoints3.projectives, geometry3_hyp_table);
